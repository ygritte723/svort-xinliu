...
if __name__ == "__main__":
    ...
    atlas_dir = './CRL_FetalBrainAtlas_2017v3/'
    out_dir = './registered'
    ...
    for n in range(1, 81):
        ...
        atlas = ants.image_read(f_atlas)
        masked_img_ants = ants.from_numpy(img_arr, spacing=(1, 1, 1), origin=(0, 0, 0), direction=img.affine[:3, :3])


        # reg = ants.registration(atlas, masked_img, type_of_transform='Similarity')
        reg = ants.registration(atlas, masked_img_ants, type_of_transform='Similarity')
        ...
        img_ants = ants.from_numpy(img.get_fdata(), spacing=(1, 1, 1), origin=(0, 0, 0), direction=img.affine[:3, :3])
        warped_img = ants.apply_transforms(fixed=atlas, moving=img_ants, transformlist=reg['fwdtransforms'])
        ...
        for label in range(int(seg_arr.max())+1):
            ...
            seg_label = (seg_arr == label).astype(np.float32)
            seg_label_ants = ants.from_numpy(seg_label, spacing=(1, 1, 1), origin=(0, 0, 0), direction=seg.affine[:3, :3])

            # warped_seg_label = ants.apply_transforms(fixed=atlas, moving=seg_label, transformlist=reg['fwdtransforms'])
            warped_seg_label = ants.apply_transforms(fixed=atlas, moving=seg_label_ants, transformlist=reg['fwdtransforms'])
